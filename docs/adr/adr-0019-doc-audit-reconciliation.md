...
backend capability without full UI exposure, and planned/not implemented work.

The selected decisions are:

| Finding | Decision |
|---------|----------|
| startup restore | Implement bounded restore of only the last project path and active map reference/path when they still exist. |
| timestamp | Render existing track point timestamp values in the point list without changing sorting semantics. |
| color | Expose the existing track color mutation through compact track UI controls. |
| line width | Expose the existing track line width mutation through compact track UI controls. |
| waypoint symbol | Route committed symbol changes through one undoable `ProjectCommand` per change. |
| OK-standard | Show warning-only UI validation for `YYYYMMDD_Callsign`; do not block rename, save, or export. |
| 10-Tracks | Use active-bundle `10-Tracks/` paths as GPX/PLT export dialog defaults when available; users may still choose another path. |
| multi-layer | Add active track-layer and waypoint-layer selection for existing workflows; do not build full layer management. |
...
- Later docs can distinguish implemented behavior from backend-only capability and
  planned/not implemented work without reopening the audit decisions.
- The feature-status table reduces future ambiguity between backend capability and UI
  exposure.

### Negative

- Public docs need ongoing maintenance whenever backend capability and UI exposure diverge.
