...
| P1 | **Task 9a: Save/Load project (.ozp)** | pending | [smoke-project-save-load.md](smoke-project-save-load.md) | Verify project persistence: save dialog reachable (Cmd+S, File menu), file created at requested path (.ozp format), project reopens with all tracks/waypoints/state restored (positions, colors, zoom). Success: all state restored identically. |
| P1 | **Task 9b: Recent projects list** | pending | [smoke-project-recent.md](smoke-project-recent.md) | Verify Recent Projects UI (File → Open Recent or sidebar button) per ADR-0020 MVP-must. Expected: list shows ≥3 recent projects in reverse chronological order; clicking opens correct project. High probability of `missing` (only last-project auto-restore implemented per docs/persistence-session.md). |
| P1 | **Task 9c: Undo/Redo** | pending | [smoke-undo-redo.md](smoke-undo-redo.md) | Verify Undo (Cmd+Z) and Redo (Cmd+Shift+Z/Cmd+Y) for waypoint move, waypoint delete, and track point delete. Each undo/redo is single step (drag events coalesced). Success: all action types support undo; multi-step undo/redo works. |
...
| P2 | **Task 5e: Track color and line-width styling** | pending | [smoke-track-style.md](smoke-track-style.md) | Style controls (color picker, width slider) expected in Tracks panel or context menu. Awaiting Appium run to verify color picker accessibility, color change application, and persistence. |
...
| P2 | **Task 9d: OK-standard track name validation** | pending | [smoke-ok-standard.md](smoke-ok-standard.md) | Verify track name validation for YYYYMMDD_Callsign format (per LizaAlert OK standard). Non-conforming name triggers warning icon or message; warning disappears on valid name. Success: validation visible and works; format specified in warning. |
...
| P3 | **Task 8a: Distance measurement tool** | **missing** | [smoke-tool-distance.md](smoke-tool-distance.md) | No distance measurement button, panel, or context menu found in AX tree. ADR-0020 on-map measurement tool not implemented in current MVP. |
| P3 | **Task 8b: Circle with explicit radius** | **missing** | [smoke-tool-circle.md](smoke-tool-circle.md) | No circle drawing tool or radius input UI found in toolbar/sidebar/menus. ADR-0020 on-map construction tool not implemented in current MVP. |
| P3 | **Task 8c: Waypoint projection** | **missing** | [smoke-tool-projection.md](smoke-tool-projection.md) | No projection context menu option or dedicated projection button found. ADR-0020 waypoint projection feature not implemented in current MVP. |
| P3 | **Task 9e: Theme selection (Catppuccin)** | pending | [smoke-themes.md](smoke-themes.md) | Verify theme picker UI (sidebar or settings) and all 5 themes: Latte (light), Frappé (dark), Macchiato (dark), Mocha (dark), Auto (system). Each theme applies visually; theme persists across relaunch. High probability of `missing` if not implemented. |
| P3 | **Task 9f: Dev console + FPS counter** | pending | [smoke-devtools.md](smoke-devtools.md) | Verify dev console toggle (backtick key) and FPS counter toggle (F3 key). Console displays; FPS shows numeric frame rate in corner. Both can be toggled independently. Partial if only one works. |
