...
- **Объекты карты**
  - Waypoints (свойства, вложения, список, проекция точки) — **[ЯДРО]** ✅; вложения с 23.09 — список путей к файлам рядом с проектом, а не одна ссылка
  - Tracks (Track Control, List, Replay, Profile, Filter, проекция точки) — **[ЯДРО]** ✅ (Filter = упрощение Дугласа–Пекера с превью; Replay — с 23.09); **Move** — нет и не планируется: трек это запись, а не чертёж
...
| Точки (waypoints) | **Ядро** | Есть: add/move/rename/symbol/цвет/заметка/вложения/visibility; импорт и экспорт `.wpt` и GPX |
...
| Track Replay (проигрывание трека во времени) | **Берём** | Есть с 23.09: ползунок по часам трека, маркер на карте, позиция интерполируется между точками, молчание длиннее пяти минут помечено как догадка |
| Track Move (сдвиг трека целиком) | Позже | Нет. В поиске почти не нужен: трек — это запись, а не чертёж |
| Вложения к точке (фото находки) | **Берём** | Есть с 23.09: список путей к файлам рядом с проектом. `.ozp` остаётся читаемым JSON; передать работу значит передать папку |
