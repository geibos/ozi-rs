...
// Wire-shape DTOs are generated by tauri-specta from the Rust registry;
// `types.ts` re-exports them under their historical frontend names.
...
describe("active layer UI wiring", () => {
  it("exposes minimal track and waypoint layer lists in app state DTOs", () => {
    ...
    expect(bindingsSource).toContain("export type LayerSummaryDto");
    expect(bindingsSource).toContain("track_layers: LayerSummaryDto[]");
    expect(bindingsSource).toContain("waypoint_layers: LayerSummaryDto[]");
    // types.ts extends the generated shape with the frontend-only
    // `visible` flag instead of redeclaring the wire type.
