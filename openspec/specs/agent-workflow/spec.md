...
## Purpose
How work on this repository is cut up and handed on: the state page an agent
reads first and writes last, the screenshot gallery that records what a slice
changed, the five checks before a slice is done, and the protocol for
verification that takes over the screen — including when to stop and hand back
a diagnostic rather than try a third time.

### Decision history

- Change `revive-ui-cycle` (2026-09-19 → 2026-09-23): `docs/STATE.md`, `docs/progress/`, the Definition of Done and the verification protocol. Rationale: three UI rewrites died where the previous one's context did, and a session that ends mid-slice left nothing a later one could pick up. Codified as the requirements in this capability.
- Not codified, deliberately: "slice equals branch equals pull request". Every slice since July has gone to `main` directly, because the owner is the only committer and a pull request against yourself buys review from nobody while costing a round trip. What the rule was for — the five checks — is `AGENTS.md` § "Before a slice is done" and `.github/pull_request_template.md`. The requirement is held in `finish-the-rebuild` for when a second person commits.
