...
- [ ] 2.3 Spot-check every ADDED requirement against its cited evidence (file:line or test); drop or fix any that codify unverified behavior. Partly done on 2026-09-21: the design's two findings lists were re-checked against the code — two of the three spec-contradicts-code items are closed, one (the unreachable theme picker) still stands, six code findings are closed and one was never true as written. The 63 requirements themselves are not yet walked one by one.
...
- [x] 2.6 Fix the documentation bugs listed in design.md "Findings" so the `documentation` scenarios pass: feature-status rows for sort/crop/split-join/ZIP, command preamble count, commands-reference completeness, `TBD` → `pending`, conventions.md concurrency, architecture.md ozf2, glossary DTO note, frontend-architecture Ctrl+Z, persistence-session.md paths, native-qa-mcp.md `qa_observe`. Done on 2026-09-21, each checked against the code first: four rows of feature-status claimed commands that exist and are reached (sort, crop by extent and by time, split/join, ZIP import); `TBD` is gone from the matrix; the command reference gained nine commands and now says the registry in `lib.rs` is the list; "no async runtime" (there is a tokio one, scoped to the download path), "no Ctrl+Z" (the layout binds it), "local ozf2 crate" (published, 0.1) and the glossary's DTO note (most are generated; only event payloads are mirrored by hand) corrected
